...
def get_dominant_pollutant(
    pm25: float | None, pm10: float | None, o3: float | None, no2: float | None, so2: float | None, co: float | None
) -> str:
    iaqis: dict[str, float] = {}
    for name, val in [("pm25", pm25), ("pm10", pm10), ("o3", o3), ("no2", no2), ("so2", so2), ("co", co)]:
        if val is not None:
            iaqis[name] = calculate_iaqi(name, val)
    if not iaqis:
        return "pm25"
    return max(iaqis, key=iaqis.__getitem__)
...
def calculate_aqi(
    ...
    dominant = max(iaqis, key=iaqis.__getitem__)
