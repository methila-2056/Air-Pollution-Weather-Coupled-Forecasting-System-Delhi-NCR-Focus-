...
Usage:
    python -m scripts.build_training_dataset [--output data/ml] [--json]
...
leak-free panel with PM2.5 lags and rolling features, splits it train/val/test,
and writes:

    data/ml/training_dataset.csv         full panel (with `split` column)
    data/ml/training_dataset.parquet     same, columnar
    data/ml/dataset_summary.json         the required dataset summary
...
def main() -> None:
    ...
    df, summary = build_training_dataset_from_db(station_names=args.stations)

    out_dir = Path(args.output)
    out_dir.mkdir(parents=True, exist_ok=True)
    df.to_csv(out_dir / "training_dataset.csv", index=False)
    parquet_path = out_dir / "training_dataset.parquet"
    try:
    ...
    print(f"Saved panel           -> {out_dir / 'training_dataset.csv'}  ({len(df):,} rows)")
    print(f"Saved summary         -> {out_dir / 'dataset_summary.json'}")
